...
static int eth0_state = -1;
static int ppp0_state = -1;
static char eth0gw[100]={0};
...
void print_ifinfomsg(struct nlmsghdr *nh)
{
	...
	if(tb[IFLA_IFNAME]) {
        if (strcmp(RTA_DATA(tb[IFLA_IFNAME]), NIC_NAME) == 0)
        {
            printf("joy:%s: %s flag[0X%x]", (nh->nlmsg_type==RTM_NEWLINK)?"NEWLINK":"DELLINK", (ifinfo->ifi_flags & IFF_LOWER_UP/*IFF_UP*/) ? "up" : "down", ifinfo->ifi_flags);
            eth0_state = ifinfo->ifi_flags & IFF_LOWER_UP/*IFF_UP*/? UP : DOWN;
            /* eth0 disable */
            if (eth0_state == DOWN)
            {
                if (strlen(eth0gw) > 0)
                {
                    sprintf(tmp, "route del default gw %s", eth0gw);
                    system(tmp);
                    sprintf(tmp, "route add default dev ppp0");
                    system(tmp);
                    sprintf(tmp, "/etc/init.d/loragw restart");
                    system(tmp);
                }
            }
            else
            ...
            /* eth0 enable */
                ...
                {
#if 0
                    sprintf(tmp, "kill -9 `ps -ef | grep \"udhcpc\" | grep -v grep|awk '{print $1}'`");
                    system(tmp);
                    sprintf(tmp, "/sbin/udhcpc ");
#else
                    sprintf(tmp, "route add default gw %s", eth0gw);
                    system(tmp);
#endif
                    sprintf(tmp, "route del default dev ppp0");
                    system(tmp);
...
void print_rtmsg(struct nlmsghdr *nh)
{
	...
	if (tb[RTA_GATEWAY] != NULL) {
        ...
        if (strncmp(tmp, "192", 3) == 0)
        {
            printf("joy:eth0  RTA_GATEWAY %s ", tmp);
            memcpy(eth0gw, tmp, strlen(tmp)+1);
        }
        else
        {
            printf("joy:ppp0 gateway %s ", tmp);
        }
	}
